...
$commands = "cd /var/www/html/app-citas && sudo chown -R ubuntu:www-data . && sudo chmod -R 775 storage bootstrap/cache database && git fetch origin main && git reset --hard origin/main && composer install --no-dev --optimize-autoloader && php artisan optimize:clear && php artisan view:cache && php artisan config:cache && sudo service php8.2-fpm restart && sudo systemctl restart nginx"
